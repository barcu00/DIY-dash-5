...
void Ui::createShiftSettings(lv_obj_t* parent) {
    ...
    for(int i=0;i<36;i++) {
        ...
        const unsigned rpm=settings.rpm_scale_max*(2*i+1)/72;
        lv_obj_set_style_bg_color(block,rpm>=settings.shift.red_rpm ? UiTheme::red():rpm>=settings.shift.start_rpm ? UiTheme::yellow():UiTheme::green(),0);
    }
    ...
    const uint16_t initial[]={settings.rpm_scale_max,settings.shift.start_rpm,settings.shift.red_rpm,settings.shift.flash_rpm,settings.shift.max_rpm};
    for(int i=0;i<5;i++) {
    ...
    if(settings.shift.flash_enabled)lv_obj_add_state(shift_flash_enabled_,LV_STATE_CHECKED);
...
void Ui::refreshShiftControls() {
    ...
    for(int i=0;i<36;i++)if(rpm_preview_blocks_[i]) {
        const unsigned rpm=settings.rpm_scale_max*(2*i+1)/72;
        lv_obj_set_style_bg_color(rpm_preview_blocks_[i],rpm>=settings.shift.red_rpm ? UiTheme::red():rpm>=settings.shift.start_rpm ? UiTheme::yellow():UiTheme::green(),0);
    }
    lv_obj_t* sliders[]={rpm_scale_slider_,shift_start_,shift_red_,shift_flash_,shift_max_};
    lv_obj_t* labels[]={rpm_scale_value_,shift_start_value_,shift_red_value_,shift_flash_value_,shift_max_value_};
    const uint16_t values[]={settings.rpm_scale_max,settings.shift.start_rpm,settings.shift.red_rpm,settings.shift.flash_rpm,settings.shift.max_rpm};
    for(int i=0;i<5;i++) {
    ...
    if(settings_status_)lv_label_set_text(settings_status_,settings.shift.flash_rpm>settings.rpm_scale_max ? "FLASH threshold is beyond the visible RPM scale":"Colors follow YELLOW / RED thresholds; 12 LEDs retain 4/4/4 zones");
}
