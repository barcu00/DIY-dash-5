...
void Ui::createShiftSettings(lv_obj_t* parent) {
    ...
    const char* labels[]={"RPM SCALE MAX","YELLOW FROM","RED FROM","FLASH FROM"};
    lv_obj_t** sliders[]={&rpm_scale_slider_,&shift_start_,&shift_red_,&shift_flash_};
    lv_obj_t** values[]={&rpm_scale_value_,&shift_start_value_,&shift_red_value_,&shift_flash_value_};
    const intptr_t actions[]={RpmScaleChanged,ShiftStartChanged,ShiftRedChanged,ShiftFlashChanged};
    const uint16_t initial[]={config_->rpm_scale_max,config_->shift.start_rpm,config_->shift.red_rpm,config_->shift.flash_rpm};
    for(int i=0;i<4;i++) {
